...
var CONST_AJAX_URL = '/cabinet/ajax-0-0-7?typecabinet=' + readCookie('typecabinet') + '&colorcabinet=' + readCookie('colorcabinet') + '&type_door=' + readCookie('type_door') + '&color_door=' + readCookie('color_door') + '&id=';
...
function setDefault(kitchenID) {
    ...
    $('.colors ul li:first-child div').each(function () {
        ...
        // уже были тут картинку выбирали??
        //alert(readCookie('imgnuminlistcabinet'));
        if (readCookie('numimage_handle')) {
            idColor = readCookie('numimage_handle');
        }
        findColor(kitchenID, i, idColor);
        $('#' + CONST_FIELD_TYPE + i).val(idColor);
